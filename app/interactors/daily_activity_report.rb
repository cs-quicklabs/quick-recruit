class DailyActivityReport < Patterns::Service
  def call
    User.recruiters.each do |recruiter|
      RecruiterMailer.with(recruiter: recruiter,
                           ...
                           pipelined_candidates_count: pipelined_candidates_count(recruiter),
                           ...
                           pipelined_candidates_daily_report_url: pipelined_candidates_daily_report_url(recruiter))
  ...
  def pipelined_candidates_count(recruiter)
    params = { owner_id: recruiter.id.to_s, bucket_updated_after_date: Date.yesterday.to_formatted_s(:db), bucket_updated_before_date: Date.yesterday.to_formatted_s(:db), bucket: :pipeline }
  ...
  def pipelined_candidates_daily_report_url(recruiter)
    "#{Rails.application.config.default_url_options[:host]}/report/update_candidates?owner_id=#{recruiter.id.to_s}&bucket_updated_after_date=#{Date.yesterday.to_formatted_s(:db)}&bucket_updated_before_date=#{Date.yesterday.to_formatted_s(:db)}&bucket=pipeline"
  end
end
